...
- **Browser side:** `lib/push-client.ts` registers `public/sw.js` for one portal at a time. A `PushTarget` carries the four things the two portals differ in — the scope, what the app is called on an iPhone's Home Screen, what records a subscription and what gives one up — so the parts that are easy to get wrong are written once: the iPhone wording, waiting for the worker, and replacing a subscription made under an older VAPID key. `EMPLOYEE_PUSH` (`/employee`) is the default, so every existing `enablePush(key)` call is unchanged; `ADMIN_PUSH` is `/admin`. The service worker only shows pushes and opens their URL; it caches nothing.
  ...
  - **The team group is deliberately not pushed to the manager.** They are in every one of them, so it would put the whole studio's chatter on their phone; private messages and calls are.
