...
- **Employees put things on projects too** (`/employee/projects`, `lib/actions/employee-project-actions.ts`): drawings, documents, and photos into a room of the gallery. A Projects tab in the portal lists every non-archived project; tapping one gives what is on it and the forms to add more.
  ...
  - ~~**Adding only — nothing there deletes.**~~ **Reversed by the studio.** The team now works a project exactly as the manager does — the same eleven tabs, the same edits, and removing a file as well as adding one. The old rule said an accidental tap on a phone should not be able to take a drawing off a client's page for good; the studio decided the team needs to fix their own mistakes more than they need protecting from them. Every delete still asks first.
  - **The team's project screens are the manager's modules, re-exported** (`/employee/projects/[id]/…`, eleven tabs). Not copies: two implementations of the same screen is two things to keep in step, and one of them drifts. `ProjectTabs` takes a `portal` so the same tabs point at either side.
    - **Two tabs are written out rather than re-exported, each for a reason.** *Overview* drops the Danger Zone, because `deleteProject` deliberately stayed manager-only and a button that throws `Unauthorized` reads as a broken platform rather than a boundary. *Gallery* keeps the employee portal's own uploader, because the admin's posts several files in one request and the body limit applies to the raw upload — which is the documented failure that lands on exactly the person this is for, somebody on a site with eight camera photos.
  - **`requireStaff` is what lets them in** (`lib/admin-guard.ts`, beside `requireAdmin`): the manager **or** somebody on the team, and nobody else. `requireAdmin` was not loosened — widening it would not have meant "employees too", it would have meant everything — and the guard was not copied into eleven action files, which is the arrangement that produced twenty-five drifting copies before.
    - **What stayed the manager's:** creating and deleting a project, the company timezone, linking or unlinking the studio's WhatsApp number, a free-text test message, and the deductions run — plus payroll, employees, settings, the week board, automation and warnings, which were never project actions at all.
    - `tests/admin-guard.test.ts` counts both guards per file, pins the particular exports where the *choice* is the decision (`deleteProject` admin, `sendProjectWhatsApp` staff), and asserts that **no manager-only action file mentions `requireStaff`** — the one regression this change made possible, and one that would typecheck, lint, build and pass every other test on its way through.
  - **Every project action refreshes both portals** (`lib/project-paths.ts` `refreshProject`). Each file used to revalidate `/admin/projects/<id>/<tab>` alone, which was right while only the manager could open those screens: afterwards, an employee deleting a photo would refresh the manager's copy and not the one in front of them, so the photo stayed on screen until a reload — reading as "delete is broken" while the database was perfectly correct.
...
- **Attendance from the fingerprint device** (ZKTeco **UF200-S** on the studio wall, `ATTENDANCE_DEVICE_IP`). `AttendanceRecord` was built for this from the start — one row per person per day, `delayHours`, and a `source` column — so the device fills in the number the manager used to type, and payroll reads it unchanged.
  - `lib/attendance.ts` is the arithmetic, pure and tested because it decides money: the first read of a day is the arrival, lateness is measured against `work-hours.ts`, arriving early is never negative credit, and a day nobody works produces **no row at all** rather than a zero one that would still count as a day of attendance.
  - **The studio's lateness rule: an allowance, then whole hours, rounded up.** `WorkHours.graceMinutes` (5 here, edited in Settings beside the hours) is taken off first, so arriving by 11:05 on an 11:00 day is not late at all; past it, `lateHours` charges `Math.ceil`, so 11:06 is one hour, 12:40 is two and 14:20 is four. Asked for deliberately — the point is that lateness costs something worth avoiding, not that the arithmetic is exact — and anything partial rounds against the latecomer.
    - **The hours are counted from the end of the allowance, not from the start of the day**, or 12:05 with five minutes' grace would round to two hours instead of one and overcharge every late morning by an hour. A test pins that one case.
    - `graceMinutes` defaults to **0** for an installation that has not chosen, because a default that invented an allowance would hand back money nobody agreed to give. And `attendanceFromPunches` takes its grace from the `WorkHours` it is handed rather than from that constant, so the sync and `check-attendance-device.mts` — which both call it with three arguments — cannot silently charge from the first minute.
    - It applies to what the device records **from now on**. Nothing already written is recalculated: the sync's cutoff is today, so a settled figure never moves under somebody.
  - Punches are grouped by the calendar day **in the company timezone**. Amman is three hours ahead of UTC, so grouping on the raw timestamp files a late arrival under the wrong day and, at month end, the wrong payslip.
  - **A manager's figure always wins.** `mayDeviceWrite` lets a sync create a row or update one it wrote itself, and never touch a `MANUAL` one — somebody typed it *because* the device was wrong. Editing a device row in the admin re-marks it MANUAL, so it is protected from then on.
  - **People are paired by the device's user number** (`Employee.deviceUserId`, unique), never matched by name. The first time this ran, the device knew Hamed, Hanin, Sally, Sofian and Abdd while the platform knew Wael, Sally, Salem and Amro — one name in five. A name match would deduct from the wrong person and leave nothing to show why. Unpaired numbers are reported by the sync, never guessed at.
  - **A sync defaults to today** (`cutoffFor`). The device held two years of logs; writing them would have put lateness into payslips already paid, because payroll sums by period. Absent or malformed means today — reaching further back has to be spelled out.
  - **It refuses to write when the device's clock is out** by more than ten minutes (`MAX_DRIFT_SECONDS`). This one was found set to **November 2000** with a nine-month hole in its log — a dead backup battery. A wrong clock is a fault to report, not data to record.
  - Runs on the existing cron endpoint (`?job=attendance`) and from a Sync now button on `/admin/payroll`. The device hands over its whole log every time and each day lands on the same `(employee, day)` row, so repeating a sync writes the same numbers rather than doubling anything, and nothing is ever deleted from the device.
  - **`scripts/check-attendance-device.mts` answers "why is attendance wrong" without changing anything**: reachable, clock drift, who is enrolled, who is paired, and what a sync would record. Run it before editing code.
  - **`/admin/attendance` is the device's own screen**: whether it answers, what its clock reads, who is enrolled, who those people are here, what it has recorded, and what a manager can do to it from a desk — sync today, set its clock, add somebody, remove somebody, wipe its log. It reads the device on render, which the payroll screen deliberately does not, and degrades rather than fails when the thing is unplugged.
    - **`deleteUser` takes the device's internal `uid`, not the `userId`** that the attendance log and `Employee.deviceUserId` use — and they differ: a user created here as number 97 came back as uid 6. `readDeviceUsers` carries the uid for exactly this reason. Delete by the wrong number and you remove a different person, with nothing on screen to say so.
    - **`setUser` creates the record, not the fingerprint.** A finger can only be enrolled at the machine by the person themselves; nothing over the network can do it. Any screen offering "add" has to say so, or a manager walks away believing somebody is set up when the device will never recognise them.
    - **Wiping the log is irreversible and destroys history that was never imported**, since a sync only records days from its cutoff onward. It is guarded by a typed `WIPE` rather than a boolean so a stray call with a default argument cannot do it.
    - **The month is a grid, not a list** (`lib/attendance-month.ts`, pure and tested; `components/admin/attendance-month.tsx`): people down the side, every day across the top, `?month=YYYY-MM` to move. It replaced a list of the last fifteen rows, which could say what came in recently and never "was anybody in on the 3rd" — the question a month is looked at for. Working days come from `isWorkingDay`, so the shading follows Settings rather than a weekend written into the component.
      - **An empty cell means nothing was recorded, never that somebody was absent.** The platform cannot tell "did not come" from "the device did not read them" from "nobody has synced" — the same refusal the day board makes about silence — and the screen says so under the grid, because a field of blanks is exactly the shape that invites the other reading.
      - Month maths is pure for the usual reason: a 28-day February, a year turning over and a day key built from the server's clock all go wrong silently, putting somebody's Tuesday in Monday's column.
  - **The library's Dates are wall clock, never instants.** `zkteco-js` builds them from the numbers the machine sends, interpreted in the *running process's* timezone — right on a laptop in Amman, three hours out inside the UTC container. `readPunches` had the same flaw, so every 11:00 arrival would have read as 14:00 and charged three hours of lateness to everybody, every day. It looked perfect in development; the live job refusing with `driftSeconds: 10799` (2h59m59s, exactly Amman's offset) is what exposed it. Always go through `deviceWallClock` and place the result with `instantAt`, and build the Date for `setTime` from the studio's wall clock rather than passing an instant through.
    - Keep the **seconds**: `instantAt` speaks HH:MM, and dropping them made a correct clock report as up to 59 seconds slow — a fault invented by the reading, which was briefly mistaken for a dying battery before it was measured twice ninety seconds apart.
...
- **Live refresh:**
  ...
  - **Presence is that same open connection.** Holding `/api/live` open *is* what says somebody is here, so the route writes a heartbeat for whoever opened it — every `HEARTBEAT_MS` (30 s) rather than on every 2 s poll — into `ChatPresence`, one row per person keyed like `ChatRead.readerKey`: `"admin"` for the manager, the employee id otherwise. Who is here travels back as its own `people` event, `LiveSync` publishes it to `lib/use-presence.ts`, and any screen reads it through `usePresence()` (a `useSyncExternalStore` store, for the same reason `use-minute-now` is one).
    - **It is deliberately NOT in `liveSignature()`.** That signature is what makes every open page re-render; a heartbeat inside it would redraw the whole platform, for everybody, every few seconds.
    - **The windows live in `lib/presence.ts`** (pure, `tests/presence.test.ts`): online for 75 s after a beat — longer than two beats, so a phone that slept for a moment is not called away — and typing for 7 s after a keystroke. `lastSeenLabel` returns **null** for somebody never seen, so a screen says nothing rather than "Offline": not knowing is not the same as being away, the same refusal the day board makes about silence.
    - **Typing rides the same row** (`typingChannelId` + `typingAt`), written by `POST /api/chat/typing` at a four-second cadence rather than per keystroke, and read by the chat stream's `people` event. Nothing sweeps it: a note nobody refreshes falls outside the window when it is read, so stopping and closing the tab look the same — and both are true.
    - **Read ticks are per conversation, not per message.** `ChatRead.lastReadAt` vs the message's `createdAt` (`isReadBy`) colours the second tick in a **private chat only**; in the team group `readAt` is null and the ticks stay at "Sent", because one pair of ticks cannot honestly mean "everybody".
    - This is presence tracking of the studio's own staff, and it was asked for explicitly. It is the one place the platform records when somebody is at their device — note that `performance-card.tsx` still measures none of it, and the day board still refuses to read silence as a verdict.
...
- **The employee portal is a phone below `lg` and a desk above it**, the same shape the admin has always had: a dark rail on the left (`components/employee/employee-rail.tsx`) beside a wide column, instead of a 672px strip stranded in the middle of a monitor with a row of thumb-sized tabs along the bottom.
  - **The frame itself did not change, and must not.** `.admin-shell` is fixed and sized to the visible viewport too, and fills a desktop perfectly well — so what made the portal narrow was the `max-w-2xl` in its layout and the tab bar, not the frame. Releasing the frame to "fill the page" would break the thing it exists for: keeping the tab bar and the message box above the iOS keyboard.
  - **The width lives on `.employee-main` itself, never on a wrapper inside it.** `globals.css` has `.employee-main:has(> .fills-frame)`, which the chat depends on for its padding and scrolling, and that selector matches a **direct child** — so the obvious way to centre a column, a `<div className="mx-auto max-w-5xl">` around `{children}`, silently stops the chat filling the screen. It would look fine on every other page.
  - **One list of destinations, two presentations.** `EMPLOYEE_DESTINATIONS` is exported from the tab bar and imported by the rail, because two copies are two things to keep in step and the one that drifts is the one nobody is looking at.
...
- **Browser side:** `lib/push-client.ts` registers `public/sw.js` for one portal at a time. A `PushTarget` carries the four things the two portals differ in — the scope, what the app is called on an iPhone's Home Screen, what records a subscription and what gives one up — so the parts that are easy to get wrong are written once: the iPhone wording, waiting for the worker, and replacing a subscription made under an older VAPID key. `EMPLOYEE_PUSH` (`/employee`) is the default, so every existing `enablePush(key)` call is unchanged; `ADMIN_PUSH` is `/admin`. The service worker only shows pushes and opens their URL; it caches nothing.
  - **The manager's own phone** is the switch on `/admin/settings` (`components/admin/admin-push-toggle.tsx`, `lib/actions/admin-push-actions.ts`). Until it, every notification addressed to the manager waited in a chat list until they next looked: `savePushSubscription` is behind `requireEmployee`, and the manager cannot sign in to the employee portal at all, so there was no action they could reach and no row to record a device against.
  - **What made them addressable is the employee row the attendance device is paired to** (`accessRole: "MANAGER"`). `lib/manager-account.ts` is the one place that looks it up on purpose — every other query means "the team" and excludes it. Without that row there is nobody to tell: `notifiableEmployeeId` returns null, the card says which pairing to do rather than failing on the tap, and a call still rings everybody else.
  - **A notification for the manager must never carry an `/employee` link.** That portal refuses them, and a notification opening a page that turns you away reads as a broken platform. `adminChatUrl` is the mirror of `employeeChatUrl`, and a test pins that nothing addressed to the manager starts with `/employee`; a chat between two employees gives the list, since `mayOpen` refuses it to the manager anyway.
  - **What reaches the manager's phone, and what deliberately does not.** A private message, an incoming call, a missed call and a meeting's reminder do. The team group does not — they are in every one of them, so it would put the whole studio's chatter on a phone all day. Nor does a meeting they have just set or just called off: they are the one who did it, and the card is in front of them. Each exclusion is a decision, and the comment at each site says so — the point being that the old ones were not decisions but the absence of anywhere to send it, and the two are indistinguishable from the code alone.
...
- **A fifth relation in one Prisma `select` closes the local database's connection.** `messageSelect` reads the project, a task card, a call and a meeting card. Adding reactions as a fifth made every `listMessages` call fail with "Server has closed the connection", from a database that answered a plain query at the same moment — the same signature as the dead database above, and not one.
  - **It is the count, not the query.** Measured one piece at a time: the two new scalars alone pass, the reactions relation alone passes, that relation with an `orderBy` passes, and only all five together fall over. So the thing to be careful about is widening a select that already carries several relations, whatever is being added to it.
  - **Which tests fail is the tell.** A per-file run — the documented recovery above — showing failures concentrated in exactly the files that touch the widened query, with no `expected`/`actual` diff anywhere, is a regression, not the local database dying under load. Load scatters failures and cancels whatever was queued; this puts them on a bullseye. Believing the comfortable explanation here would have pushed a real regression as "the known environmental flake".
  - The fix is to read the extra relation **beside** the message rather than inside it (`reactionSnapshot` in `lib/chat-reaction-store.ts`), which is the better shape anyway: a reaction changes without its message changing, so it travels as its own live-stream event, and no other caller of `messageSelect` — notifications, the conversation list, the assistant's read — has any use for it.
- **A prop left out of the destructuring becomes a DOM global instead.** `ChatHeader` declared `status?: string | null` in its props type and never bound it, so `{status ?? subtitle}` read `window.status`. Every conversation page in both portals threw `ReferenceError: status is not defined` on the server and fell back to client rendering — for a whole release, because on screen it just looked unfinished.
  - **Nothing we run catches it.** `tsc` is happy: `lib.dom.d.ts` declares `var status: string` globally, so the identifier resolves. Lint, the tests and `npm run build` all pass. In the browser the page still draws, with that line silently blank — which reads as "presence isn't wired up yet", not as a crash. The only witness is `⨯ ReferenceError` in the dev server's log, so **read the dev log after adding a prop**, not just the page.
  - The globals that can swallow a prop this way include `status`, `name`, `length`, `top`, `closed`, `origin`, `event`, `history` and `location`. When a prop with one of those names appears not to arrive, check the destructuring before suspecting the caller.
...
- ~~**Arabic breaks `gallery.pdf`.**~~ Fixed. pdf-lib's standard fonts are WinAnsi-encoded, and handed an Arabic letter `drawText` does not draw a box — it **throws**, so the route answered 500 after sixteen seconds of fetching and re-encoding every render in the gallery, because one room was called الطابق الاول.
  - **Text it cannot encode is now drawn as a picture of itself**, rendered through sharp — which is libvips, which is pango and harfbuzz — so Arabic comes out **joined and right-to-left**. The obvious alternative, embedding a Unicode font with `@pdf-lib/fontkit`, was rejected after checking what it does: fontkit draws the glyphs but performs no shaping and no bidi, so it produces a PDF that builds successfully and is still wrong to anybody who reads Arabic.
  - **The slim image ships no fonts at all** — libvips drew an empty box per character, for Latin as much as Arabic, *while reporting success*. `fontconfig` and `fonts-noto-core` are installed in the Dockerfile for this; without them the route falls back to drawing what Helvetica can and omitting the rest, so a machine with no fonts still produces a gallery rather than nothing.
  - `drawLine` is the single place that chooses between the two paths, and nothing in it is allowed to throw: ninety renders must not be lost to one character in a caption. Pango takes *markup*, so the text is escaped — a room called "Kitchen & Bar" would otherwise fail to parse and lose its line.
  - Zip and PDF **filenames** still drop non-ASCII characters; that is the `Content-Disposition` name only, not the document.
