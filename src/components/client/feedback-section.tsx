...
export function FeedbackSection({ token, comments }: { token: string; comments: CommentRow[] }) {
  ...
  return (
    <SectionShell id="feedback">
      <SectionHeader eyebrow="Communicate" title="Review & Feedback" description="Have a note or a change request? Send it directly to NEON." />
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-5">
        ...
        >
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-ink/50">Referring to (optional)</label>
            <input
              name="refLabel"
              placeholder="e.g. Kitchen Render"
              className="w-full rounded-lg border border-ink/15 bg-white/70 px-3 py-2 text-sm outline-none focus:border-cyan-strong"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-ink/50">Message</label>
            <textarea
              name="message"
              required
              rows={4}
              placeholder="Change this sofa to a darker color…"
              className="w-full rounded-lg border border-ink/15 bg-white/70 px-3 py-2 text-sm outline-none focus:border-cyan-strong"
            />
          </div>
          <button type="submit" disabled={pending} className={buttonClasses("primary", "md")}>
            {pending ? "Sending…" : "Send to NEON"}
          </button>
        </form>
        ...
        <div className="flex flex-col gap-3 lg:col-span-3">
          {comments.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-ink/12 bg-ink/[0.02] px-8 py-14 text-center">
              ...
              <p className="mt-3 text-sm text-ink/50">No messages yet — your feedback will appear here.</p>
            </div>
          ) : (
            comments.map((c) => (
              <div key={c.id} className="glass rounded-2xl p-5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-ink">{c.authorType === "ADMIN" ? "NEON Team" : c.authorName}</p>
                    {c.refLabel && <Badge tone="cyan">{c.refLabel}</Badge>}
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge tone={c.status === "OPEN" ? "warning" : "success"}>{c.status === "OPEN" ? "Open" : "Resolved"}</Badge>
