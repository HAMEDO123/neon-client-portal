  ...
  return (
    ...
    <div className="admin-shell flex overflow-hidden bg-paper">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <div className="admin-topbar flex items-center justify-between border-b border-warm-line bg-paper-soft/90 px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] lg:hidden">
          <span className="leading-tight">
            <span className="block font-display text-base font-semibold text-bark">NEON</span>
            <span className="block text-[9px] font-semibold uppercase tracking-[0.24em] text-clay-deep">
              Interior Design
            </span>
          </span>
          <button
            ...
            className="relative rounded-lg p-2 text-bark/60 hover:bg-clay-soft/60"
          >
            <Menu size={20} />
            {waiting && (
              <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-clay ring-2 ring-paper-soft" aria-hidden />
            )}
          </button>
        </div>
        ...
        <div className="hidden items-center gap-4 border-b border-warm-line bg-paper-soft/80 px-8 py-3 lg:flex">
          <nav aria-label="Breadcrumb" className="flex min-w-0 items-center gap-1.5 text-sm text-bark/45">
            <Link
              href="/admin"
              aria-label="Dashboard"
              className="rounded-md p-1 transition-colors hover:bg-clay-soft/60 hover:text-bark"
            >
              <Home size={15} strokeWidth={1.75} />
            </Link>
            <ChevronRight size={13} strokeWidth={2} className="text-bark/25" />
            <span>Workspace</span>
            <ChevronRight size={13} strokeWidth={2} className="text-bark/25" />
            <span className="truncate font-medium text-bark">{pageName(pathname)}</span>
          </nav>
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2">
            <Link
              ...
              className="relative rounded-lg p-2 text-bark/50 transition-colors hover:bg-clay-soft/60 hover:text-bark"
            >
            ...
            <Link
              href="/admin/settings"
              className="flex items-center gap-2.5 rounded-full border border-warm-line bg-card py-1 pl-1 pr-3 transition-colors hover:bg-clay-soft/60"
            >
              <span className="flex h-8 w-8 items-center justify-center rounded-full bg-clay text-xs font-semibold text-white">
                N
              </span>
              <span className="text-left leading-tight">
                <span className="block text-xs font-semibold text-bark">NEON</span>
                <span className="block text-[11px] text-bark/45">Admin</span>
              </span>
            </Link>
          </div>
        </div>
