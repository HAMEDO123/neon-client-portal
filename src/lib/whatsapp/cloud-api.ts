// Vendored verbatim from the nexora-whatsapp library (v1.0.0, src/cloud-api.ts),
...
export type WhatsAppCloudCredentials = {
  ...
  accessToken: string;
  // Optional because a company can connect and send messages without it —
  // it's only needed to verify inbound webhook signatures. Without it,
  // verifyWebhookSignature is skipped for that company (see webhook route).
...
export async function verifyWhatsAppCloudCredentials(
  creds: Pick<WhatsAppCloudCredentials, "phoneNumberId" | "accessToken">
): Promise<{ ok: true; displayPhoneNumber: string; verifiedName: string } | { ok: false; error: string }> {
  try {
    const res = await fetch(`${GRAPH_API_BASE}/${creds.phoneNumberId}?fields=display_phone_number,verified_name`, {
      headers: { Authorization: `Bearer ${creds.accessToken}` },
    });
    const body = await res.json();
    if (!res.ok) return { ok: false, error: graphErrorMessage(body) };
...
export async function sendWhatsAppCloudMessage(creds: WhatsAppCloudCredentials, to: string, text: string): Promise<void> {
...
export async function sendWhatsAppCloudTypingIndicator(creds: WhatsAppCloudCredentials, messageId: string): Promise<void> {
  try {
    const res = await fetch(`${GRAPH_API_BASE}/${creds.phoneNumberId}/messages`, {
      method: "POST",
      headers: { Authorization: `Bearer ${creds.accessToken}`, "Content-Type": "application/json" },
...
export async function downloadWhatsAppMedia(creds: WhatsAppCloudCredentials, mediaId: string): Promise<{ buffer: Buffer; mimeType: string }> {
  const metaRes = await fetch(`${GRAPH_API_BASE}/${mediaId}`, { headers: { Authorization: `Bearer ${creds.accessToken}` } });
  if (!metaRes.ok) throw new Error(`WhatsApp media lookup failed: ${graphErrorMessage(await metaRes.json().catch(() => null))}`);
...
async function uploadWhatsAppMedia(creds: WhatsAppCloudCredentials, buffer: Buffer, mimeType: string, filename: string): Promise<string> {
  ...
  const body = await res.json().catch(() => null);
  if (!res.ok || !body?.id) throw new Error(`WhatsApp media upload failed: ${graphErrorMessage(body)}`);
...
export async function sendWhatsAppCloudVoiceMessage(
...
export async function sendWhatsAppCloudMediaMessage(
  creds: WhatsAppCloudCredentials,
  to: string,
  fileUrl: string,
  mimeType: string,
  ...
  if (!fileRes.ok) throw new Error(`Couldn't fetch the media file to send (${fileRes.status})`);
