...
export async function setMyTaskStatus(entryId: string, state: TaskState) {
  ...
  // The move itself is shared with the mobile API — canMove, the state-change
  // record and telling the manager are rules, and rules that exist twice drift.
  const result = await setEmployeeTaskState(employee, entryId, state);
